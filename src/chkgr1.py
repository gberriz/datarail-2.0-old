...
#         for _, q in the_cube.iterate_over('cytokine', 'inhibitor', 'readout:name'):
#             data = {}
...
def main():
    ...
    #     supercols.append(RegionGrid((regs,), stride, scaling_factor,
    ...
    # heatmap = RegionGrid((supercols,), stride=stride,
    ...
    #     supercolreg = RegionGrid(supercoldata, Point((xstride, ystride)),
    ...
    # grid = RegionGrid((supercols,), stride=stride,
    #                   origin=origin, quadrant=quadrant)


    inh = open('PRIV/dumps/pickle/heatmap.pkl')
    heatmap = pickle.load(inh)
    inh.close()
    inh = open('PRIV/dumps/pickle/grid.pkl')
    grid = pickle.load(inh)
    inh.close()

    scs = grid.members[0].size
    quadrant = (1, -1)
    hsep = 0.05 * scs[0]

    rowwidth = grid.size[0]
    rowheight = grid.members[0].region.members[0].size[1]
    hms = heatmap.size

    scale = numeric_vector((grid.size[0], grid.members[0].region.members[0].size[1]))/heatmap.size
    basepoint = heatmap.rect.corners[1]
    offset = grid.rect.corners[0] + Point((0, -hsep))

    trans = Transform(basepoint=basepoint, scale=scale, offset=offset)

    heatmap = heatmap.transform(trans)
    ...
    grid_labels = mk_labels(grid, (supercolnames, rownames, None, rowmaxes), hsep)
    grid.extend(grid_labels)
