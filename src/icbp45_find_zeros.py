...
__d.update(
    {
      ...
      'path_comp_attribs': 'assay plate well field'.split(),
      'encoding': 'utf-8',
      'sep': (',\t,', ',', '|', '^'),
...
def get_data_path(subrecord,
                  ...
                  _ext=PARAM.sdc_subdir_ext,
                  _basename=PARAM.sdc_basename):

    cs = map(unicode,
             [getattr(subrecord, a) for a in _comps])
    cs[-1] += _ext
    cs.append(_basename)

    return op.join(_path, *cs)
...
def main(argv):
    ...
    with open(path) as fh:
        ...
        OutputKeyCoords = namedtuple('OutputKeyCoords',
                                     KeyCoords._fields + ('cellno',))

        print_record([nt._fields for nt in OutputKeyCoords, ValCoords])
        ...
        for line in fh:
            ...
            for i, row in enumerate(rawdata):
                if any(c == 0 for c in row):
                    print_record((OutputKeyCoords(*(key + (i,))), val))
