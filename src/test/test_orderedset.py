...
class TestJointOps(unittest.TestCase):
    ...
    def test_or(self):
        ...
        self.assertEqual(self.s | list(self.otherword), i)
        self.assertEqual(self.s | tuple(self.otherword), i)
        i = set(self.s.union(self.otherword))
        self.assertEqual(set(self.s | set(self.otherword)), i)
        self.assertEqual(set(self.s | frozenset(self.otherword)), i)
        try:
    ...
    def test_xor(self):
        ...
        i = set(self.s.symmetric_difference(self.otherword))
        self.assertEqual(set(self.s ^ set(self.otherword)), i)
        self.assertEqual(set(self.s ^ frozenset(self.otherword)), i)
        try:
            self.s ^ self.otherword
    ...
    def test_do_not_rehash_dict_keys(self):
        ...
        try:
            ...
            d = dict.fromkeys(map(HashCountingInt, xrange(n)))
            sd = set(d)
            print sum(elem.hash_count for elem in sd)
            for _ in d: print id(_),
            print
            print sum(elem.hash_count for elem in sd)
            d2 = dict()
            d2.update(d)
            for _ in d2: print id(_),
            print
            print sum(elem.hash_count for elem in sd)
            for k in d2:
                d2[k] = None
            ...
            # altset = self.thetype
            # altset = Set
            # print sum(elem.hash_count for elem in set(d))
            # print sum(elem.hash_count for elem in altset(d))
            # print sum(elem.hash_count for elem in set(d))
            # print sum(elem.hash_count for elem in altset(d))
        except:
            print tb.format_exc()
...
class TestOrderedSet(unittest.TestCase):
    ...
    def setUp(self):
        ts = TestSet('setUp')
        ts.setUp()
        self.__dict__.update(kv for kv in ts.__dict__.items()
                             if not kv[0].startswith('_'))

    def test_first(self):
        self.assertEqual(self.s.first(), 's')
        self.assertRaises(ValueError, self.thetype().first)


    def test_first(self):
        s = self.thetype(self.word)
        self.assertEqual(s.first(), list(s)[0])
        s.clear()
        self.assertRaises(ValueError, s.first)

    def test_last(self):
        s = self.thetype(self.word)
        self.assertEqual(s.last(), list(s)[-1])
        s.clear()
        self.assertRaises(ValueError, s.last)


class TestSetSubclass(TestSet):
    class SetSubclass(TestSet.thetype):
        pass
...
class TestFrozenSetSubclass(TestFrozenSet):
    ...
    class FrozenSetSubclass(TestFrozenSet.thetype):
        pass
...
_skipper = unittest.skip('(not applicable)')
for cls in TestFrozenSet, TestFrozenSetSubclass:
    for name in dir(cls):
        ...
        if not callable(attrib):
            continue
        setattr(cls, name, _skipper(attrib))
del _skipper
