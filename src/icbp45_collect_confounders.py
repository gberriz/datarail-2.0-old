...
__d.update(
    {
      ...
      'encoding': 'utf-8',
      'sep': (u',\t,', u',', u'|', u'^'),
      'confcube_extra_dim_name': u'confounder',
      'datacube_extra_label': ((u'stat', (u'mean', u'stddev')),),
        # datacube_extra_label is set to a 1-tuple for a trivial reason: to
        # simplify (ever so slightly) the code that computes the data cubes'
        # labels
      'maskval': 0,
      'float': np.float64,
      'fillvalue': np.nan,
...
def main(argv):
    ...
    for cube in cubes.values():
        vs = cube.values()
        first = vs[0]
        for v in vs[1:]: v.copyorder(first)

    for cube in cubes.values():
        maxh = len(next(cube.sortedkeysmk()))
        for h in range(1, maxh):
            assert len(set([tuple(v._keyorder) for v in
                            cube.sortedvaluesmk(height=h)])) == 1

    bricks = dict()
    for subassay, cube in cubes.items():
        keys_tuple = list(cube.sortedkeysmk())
        labels = get_labels(kcoords, keys_tuple) + \
                 ((PARAM.confcube_extra_dim_name, vcoords),)
    ...
    with h5h.Hdf5File(outpath, 'w') as h5:
        ...
        for subassay, conf_hyperbrick in bricks.items():

            # NOTE: the following creates two hyperbricks: one with
            # confounder metadata and a second one, which is empty,
            # ready to receive the data generated by IR.  This scheme
            # is very convenient here, but it is FUNDAMENTALLY FLAWED,
            # because it depends on the coincidence (or rather,
            # near-coincidence) in shape between the two cubes, which
            # need not hold in general...

            datacube_labels = conf_hyperbrick.labels[:-1] + \
                              PARAM.datacube_extra_label
            datacube_shape = conf_hyperbrick.data.shape[:-1] + \
                             (len(PARAM.datacube_extra_label[0][1]),)
            datacube_data = np.ndarray(datacube_shape, dtype=PARAM.float)

            # neither the next step, nor its details, are not essential; in
            # particular, the chosen fillvalue (NaN) was just the first one
            # that came to mind.
            datacube_data.fill(PARAM.fillvalue)

            empty_hyperbrick = hb.HyperBrick(datacube_data, datacube_labels)

            for d, b in ((dir0, conf_hyperbrick), (dir1, empty_hyperbrick)):
